...
const Navbar = ({
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case "success":
        return <Check className="h-4 w-4 text-green-500 dark:blue-glow" />;
      case "warning":
        return <AlertTriangle className="h-4 w-4 text-yellow-500 dark:orange-glow" />;
      case "info":
      default:
        return <Info className="h-4 w-4 text-blue-500 dark:blue-glow" />;
    }
  };
  ...
  return <header className={`fixed top-0 left-0 right-0 bg-white/95 dark:bg-gray-900/95 dark:border-gray-800 border-b z-30 animate-fade-in backdrop-blur-lg transition-all duration-300 ${scrolled ? 'shadow-md dark:fixed-header' : ''}`}>
      <div className="flex items-center justify-between h-16 px-4 md:px-6 relative">
        {/* Left section */}
        <div className="flex items-center gap-4">
          {isMobile && <Button variant="ghost" size="icon" onClick={onMenuClick} className="md:hidden hover:scale-110 transition-all duration-300">
        ...
        <div className="flex items-center gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon" className="relative hover:scale-110 transition-all duration-300">
                ...
                {unreadCount > 0 && <span className="absolute -top-1 -right-1 h-5 w-5 bg-red-500 rounded-full text-white text-xs flex items-center justify-center animate-pulse-subtle">
                    {unreadCount}
                  </span>}
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-80 dark:bg-gray-800 dark:border-gray-700 dark:shadow-[0_5px_15px_rgba(0,0,0,0.5)] animate-slide-up" align="end">
              ...
              <div className="max-h-[300px] overflow-y-auto scrollbar-thin">
                {notifications.length > 0 ? notifications.map(notification => <DropdownMenuItem key={notification.id} className="p-0 focus:bg-transparent">
                      <div className={`w-full p-3 hover:bg-gray-100 dark:hover:bg-gray-700/50 cursor-pointer transition-colors duration-300 ${!notification.read ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                    ...
                    </DropdownMenuItem>) : <div className="py-6 text-center">
          ...
          {isLoggedIn ? <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 dark:bg-gray-800 dark:border-gray-700 animate-slide-up">
                <DropdownMenuLabel className="dark:text-gray-200">My Account</DropdownMenuLabel>
                <DropdownMenuSeparator className="dark:bg-gray-700" />
                <DropdownMenuItem className="cursor-pointer dark:hover:bg-gray-700 dark:focus:bg-gray-700 transition-colors" onClick={() => {
              navigate('/profile');
              window.scrollTo(0, 0);
            }}>
                ...
                <DropdownMenuItem className="cursor-pointer dark:hover:bg-gray-700 dark:focus:bg-gray-700 transition-colors" onClick={handleSettingsClick}>
