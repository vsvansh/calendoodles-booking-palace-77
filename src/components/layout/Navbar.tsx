...
const Navbar = ({ onMenuClick, sidebarOpen }: NavbarProps) => {
  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 bg-white/95 dark:bg-gray-900/95 dark:border-gray-800 border-b z-30 animate-fade-in backdrop-blur-lg transition-all duration-300 ${
        scrolled ? 'shadow-md dark:fixed-header' : ''
      }`}
    >
      <div className="flex items-center justify-between h-16 px-4 md:px-6 relative">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to="/" className="flex items-center gap-2 group" onClick={() => window.scrollTo(0, 0)}>
            <Calendar 
              className="h-6 w-6 text-calendoodle-purple transition-transform group-hover:rotate-12 dark:text-calendoodle-purple/90 dark:purple-glow" 
            />
        ...
        <div className="flex items-center gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent 
              className="w-80 dark:bg-gray-800 dark:border-gray-700 dark:shadow-[0_5px_15px_rgba(0,0,0,0.5)] animate-slide-up" 
              align="end"
            >
                    ...
                    <DropdownMenuItem key={notification.id} className="p-0 focus:bg-transparent">
                      <div className={`w-full p-3 hover:bg-gray-100 dark:hover:bg-gray-700/50 cursor-pointer transition-colors duration-300 ${!notification.read ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                        <div className="flex items-start gap-2">
                          ...
                          {!notification.read && (
                            <div className="h-2 w-2 bg-blue-500 rounded-full flex-shrink-0 mt-2 dark:blue-glow"></div>
                          )}
                        </div>
                      </div>
                    </DropdownMenuItem>
              ...
              <DropdownMenuItem className="justify-center focus:bg-transparent">
                <Link 
                  to="/notifications" 
                  className="text-sm text-calendoodle-blue hover:text-calendoodle-blue/80 dark:hover:text-calendoodle-blue/90 dark:hover:blue-glow font-medium"
          ...
          {isLoggedIn ? (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56 dark:bg-gray-800 dark:border-gray-700 animate-slide-up">
                ...
                <DropdownMenuSeparator className="dark:bg-gray-700" />
                <DropdownMenuItem 
                  className="cursor-pointer dark:hover:bg-gray-700 dark:focus:bg-gray-700 transition-colors"
                  onClick={() => {
                    navigate('/profile');
                    window.scrollTo(0, 0);
                  }}
            ...
            <>
              <Link to="/login" onClick={() => window.scrollTo(0, 0)}>
                <Button 
                  variant="gradient" 
                  className="rounded-full hover:scale-105 transition-all duration-300 dark:shadow-glow-blue"
                >
                  Login
                </Button>
              </Link>
              <Link to="/register" className="hidden sm:block" onClick={() => window.scrollTo(0, 0)}>
                <Button 
                  variant="outline" 
                  className="rounded-full border-2 dark:border-gray-700 dark:hover:border-calendoodle-blue/50 transition-colors hover:scale-105 dark:hover:shadow-[0_0_12px_rgba(52,152,219,0.4)]"
                >
                  Register
                </Button>
              </Link>
